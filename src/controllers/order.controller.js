...
orderController.post = async (req, res) => {
  try {
    ...
    for (i in order) {
      if (Object.prototype.hasOwnProperty.call(order, i)) {
        const prodReg = await Product.findById(order[i].id);
        if (prodReg === null) {
          throw Error('Not found');
        }
        if (prodReg.stock < order[i].total) {
    ...
    for (const [key, value] of Object.entries(products)) {
      await Product.findByIdAndUpdate(key, {stock: value});
    }
    // Agregar los elementos comprados a la bd
    for (i in orderProducts) {
      if (Object.prototype.hasOwnProperty.call(orderProducts, i)) {
        await orderProducts[i].save();
      }
    }
    const userOrder = new Order({date: new Date(), products: orderProducts});
    await userOrder.save();
    await User.findByIdAndUpdate(req.userId, {$push: {order: userOrder}});
    res.status(200).json({
